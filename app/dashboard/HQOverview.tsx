...
export default function HQOverview() {
  ...
  const handleKakaoSend = () => {
    setToast({ message: `✅ ${selectedStore?.name} 사장님께 독려 메시지가 전송되었습니다.`, type: 'success' });
          ...
          <div className="space-y-4 flex-1">
            <div className="rounded-xl bg-white/[0.03] p-4 ring-1 ring-white/5 border-l-2 border-l-rose-500">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-xs font-bold text-white">마산합성점</span>
              </div>
            ...
            <div className="rounded-xl bg-white/[0.03] p-4 ring-1 ring-white/5 border-l-2 border-l-amber-500">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-xs font-bold text-white">강남역점</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <div className="rounded-[2rem] bg-white/[0.03] p-8 ring-1 ring-white/10 flex flex-col min-h-[320px]">
          ...
          <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-1">
            {risingStars.length > 0 ? risingStars.map((s, i) => (
        ...
        <div className="rounded-[2rem] bg-white/[0.03] p-8 ring-1 ring-white/10 flex flex-col min-h-[320px]">
          ...
          <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-1">
            {focusRequired.length > 0 ? focusRequired.map((s, i) => (
              <div key={s.id} className="flex items-center justify-between border-b border-white/5 pb-3 last:border-0 last:pb-0">
                <span className="text-sm font-bold text-zinc-300">{i + 1}. {s.name}</span>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {stats.length === 0 ? (
          <div className="col-span-full flex flex-col items-center justify-center rounded-[2.5rem] bg-white/[0.03] border-2 border-dashed border-white/10 p-16 text-center animate-in fade-in zoom-in duration-500">
            ...
            <h3 className="text-xl font-black text-white mb-2">등록된 가맹점이 없습니다</h3>
            <p className="text-sm text-zinc-500 max-w-[320px] leading-relaxed mb-8">리뷰가드 시스템을 시작하려면 초대 링크를 발급하여 가맹점주를 초대하세요.</p>
            <button onClick={handleGenerateInvite} className="px-8 py-4 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-sm font-black text-white shadow-xl shadow-violet-600/30 transition hover:scale-105 active:scale-95">지금 첫 초대 발급하기 🚀</button>
          </div>
        ) : (
          stats.map((store) => (
      ...
      {isKakaoModalOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-[2rem] bg-[#FEE500] p-8 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="rounded-2xl bg-white/70 p-5 mb-6 text-sm font-bold text-zinc-800 leading-relaxed ring-1 ring-black/5 shadow-inner">"사장님, 안녕하세요. 본사 전략팀입니다. <br /><br />현재 <strong>{selectedStore?.name}</strong>의 최근 3일 평점 데이터가 하락세로 감지되었습니다.<br /><br />본사 차원의 리뷰 대응 지원이 필요하신 경우 말씀해 주십시오. 함께 개선해 나가겠습니다."</div>
            <div className="flex flex-col gap-2"><button onClick={handleKakaoSend} className="w-full py-4 rounded-2xl bg-zinc-900 text-white font-black text-sm hover:bg-zinc-800 transition active:scale-95 shadow-lg">메시지 전송하기</button><button onClick={() => setIsKakaoModalOpen(false)} className="w-full py-4 rounded-2xl bg-black/5 text-zinc-800 font-bold text-sm hover:bg-black/10 transition">취소</button></div>
          </div>
        </div>
      )}
